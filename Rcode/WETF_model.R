#******************************************************************************
#
# Calculating grain yield using the Wang–Engel temperature function (WETF)
# 
# version: 1.0
...
Metrics_csv="combinations_statsxmodel_WETF.csv"
Combinations_csv="combinations_Yield_WETF.csv"

# GLOBAL VARIABLES
OUTPUT_DIR <- "results/WETF"
WD <- NULL
WeatherFile <- NULL
NDVIFile <- NULL
PhenoFile <- NULL
...
if (is_VPDStress==TRUE){
  Metrics_csv <- "combinations_statsxmodel_WETF_vpdStres.csv"
  Combinations_csv <- "combinations_Yield_WETF_vpdStress.csv"
  OUTPUT_DIR <- "results/WETF_vpdStress"
}else{
  Metrics_csv <- "combinations_statsxmodel_WETF.csv"
  Combinations_csv <- "combinations_Yield_WETF.csv"
  OUTPUT_DIR <- "results/WETF_noStress"
}
...
load_datasets <-function(wd='.', Weather_csv=Weather_csv, NDVI_csv=NDVI_csv,
  ...
  #                                     "RUE"=numeric(), "Tmin"=numeric(), "Topt"=numeric(), "Tmax"=numeric(), "TminFactor"=numeric(), "TmaxFactor"=numeric(),
...
preprocess_datasets <- function(NDVI_lowerThreshold, NDVIFile, PhenoFile, 
  ...
  #                #VPDTC = `Vapor Pressure Deficit at Tc [kPa]`,
...
# GPP calculation
# getGPP <- function(SolRad, RUE, Tm, iPAR, stressFactor=1){
...
# No Stress condition
calculateGPP <- function(df, RUE, TminFactor, Tmin, Topt, Tmax){
  ...
  df <- dplyr::mutate (df, 
                       ...
                       WETFTMAX = calculate_WETF_wrap(Tdaymax, Tmin, Topt, Tmax),
                       GPPTMAX_WETF = getGPP(SolRad,RUE,WETFTMAX,iPAR)
  )
  return(df)
}
# --------------------------------------------------------
...
calculateGPP_VPDStress <- function(df, RUE, Lvpd, Uvpd, SFvpd_Lthres, SFvpd_Uthres){
  df <- dplyr::mutate (df,
                       ...
                       GPPSFvpd_WETF = getGPP(SolRad,RUE,WETFTMAX,iPAR, SFvpd, is_VPDStress=T)
  )
...
getYield <- function(df, phenoVars, is_VPDStress=F){
  ...
  if (is_VPDStress==TRUE){
    yield <- df %>% summarise( SGPPSFvpd_WETF = sum (GPPSFvpd_WETF) )
    yield <- dplyr::mutate (yield, 
                            SimYield = round(SGPPSFvpd_WETF * YIELD_FACTOR, 2)
    )
  } else {
    yield <- df %>% summarise( SGPPTMAX_WETF = sum (GPPTMAX_WETF) )
    yield <- dplyr::mutate (yield, 
                            SimYield = round(SGPPTMAX_WETF * YIELD_FACTOR, 2) 
                            )
  }
  return(yield)
}
...
# --------------------------------------------------------
# Report & Chart
reportYield <- function(yield, RUE, Tmin, Topt, Tmax, TminFactor, phenoVars, data_stats, 
  ...
  if (is_VPDStress==TRUE){
    yield <- mutate (yield, RUE=RUE, Tmin=Tmin, Topt=Topt, Tmax=Tmax, TminFactor=TminFactor, TmaxFactor=TxFactor, Lvpd=Lvpd, Uvpd=Uvpd, SFvpd_Lthres=SFvpd_Lthres, SFvpd_Uthres=SFvpd_Uthres)
  } else{
    yield <- mutate (yield, RUE=RUE, Tmin=Tmin, Topt=Topt, Tmax=Tmax, TminFactor=TminFactor, TmaxFactor=TxFactor)
  }
  
  if (saveReport==TRUE){
    if (is_VPDStress==TRUE){
      fname = paste(c("Yield_RUE",RUE,"_L",Lvpd,"_U",Uvpd,"_SFvpd",SFvpd_Lthres,"-",SFvpd_Uthres,"_TmnFact",TminFactor,"_TmxFact",TmaxFactor,"_Tm",Tmin,"_Tx" ,Tmax,"_Topt",Topt,"_",TFname,".csv"), collapse = "")
    } else {
      fname = paste(c("Yield_RUE",RUE,"_TmnFact",TminFactor,"_TmxFact",TmaxFactor,"_Tm",Tmin,"_Tx" ,Tmax,"_Topt",Topt,"_",TFname,".csv"), collapse = "")
    }
  ...
  if (saveFig==TRUE){
    if (is_VPDStress==TRUE){
      subtitle = paste(c("RUE: ",RUE," - VPDmx: [L:",Lvpd,", U:", Uvpd,"] - SFvpd thresholds: [",SFvpd_Lthres,", ",SFvpd_Uthres,"] - Tm: ",Tmin," - Tx:",Tmax," - Topt: ",Topt," - Tmfact: ",TminFactor ), collapse = "")
      chart_fname = paste(c("Yield_RUE",RUE,"_L",Lvpd,"_U",Uvpd,"_SFvpd",SFvpd_Lthres,"-",SFvpd_Uthres,"_Tm",Tmin,"_Tx" ,Tmax,"_Topt",Topt,"_Tmfact",TminFactor,"_",TFname), collapse = "")
    } else {
      subtitle = paste(c("RUE: ",RUE," - Tm: ",Tmin," - Tx:" ,Tmax," - Topt: ",Topt," - Tmfact: ",TminFactor), collapse = "")
      chart_fname = paste(c("Yield_RUE",RUE,"_Tm",Tmin,"_Tx" ,Tmax,"_Topt",Topt,"_Tmfact",TminFactor,"_",TFname), collapse = "")
    }
...
chartYieldxmodel <- function(df, TFname='WETF', grp='country', subtitle, fname, saveFig=F){
  pred_fld <- c("SimYield")
...
# Get all combinations for WETF (no stress)
getAllCombinations_WETF_noStress <- function(comb_args, rtrnList=T){
  df_cmb <- do.call(expand.grid, comb_args)
  df_cmb <- df_cmb %>% dplyr::filter(df_cmb$Tmin < df_cmb$Topt)
  df_cmb <- df_cmb %>% dplyr::filter(df_cmb$Topt < df_cmb$Tmax)
  if (rtrnList==T)
    df_cmb <- split(df_cmb, seq(nrow(df_cmb)))
  return(df_cmb)
}

# Get all combinations for WETF (VPD stress)
getAllCombinations_WETF_vpdStress <- function(comb_args, rtrnList=F){
  df_cmb <- do.call(expand.grid, comb_args)
...
createDF <- function(yield, RUE, TminFactor, Tmin, Topt, Tmax, pred_fld, is_VPDStress=F,
                     Lvpd=NA, Uvpd=NA, SFvpd_Lthres=NA, SFvpd_Uthres=NA
){
  TFname = "WETF"
...
metrics_combinations_WETF_noStress <- function(inputs, data, phenoVars, data_stats, saveReport=T, mergeReport=T, saveFig=F){
  RUE = inputs[['RUE']]
  Tmin = inputs[['Tmin']]
  Topt = inputs[['Topt']]
  Tmax = inputs[['Tmax']]
  ...
  TFname <- "WETF"
  pred_fld <- c("SimYield")
  dataWETF <- calculateGPP(data, RUE=RUE, TminFactor, Tmin, Topt, Tmax)
  yieldWETF <- getYield(dataWETF, phenoVars)
  r_yieldWETF <- reportYield(yieldWETF, RUE, Tmin, Topt, Tmax, TminFactor, phenoVars, data_stats, 
  ...
  df_stats <- createDF(r_yieldWETF, RUE=RUE, TminFactor=TminFactor, Tmin=Tmin, 
                       Topt=Topt, Tmax=Tmax, pred_fld=pred_fld)
...
metrics_combinations_WETF_vpdStress <- function(inputs, data, phenoVars, data_stats, saveReport=F, 
  ...
  TFname <- "WETF"
  pred_fld <- c("SimYield")
  #dataWETF <- calculateGPPandYIELD_noStress(data, TFname=TFname, RUE, TminFactor=0.25, Tmin, Toptmin, Topt, Toptmax, Tmax)
  dataWETF <- calculateGPP(data, RUE=RUE, TminFactor=TminFactor, Tmin=Tmin, Topt=Topt, Tmax=Tmax)
  #dataWETF <- calculateGPPandYIELD_VPDStress(dataWETF, TFname=TFname, RUE, Lvpd, Uvpd, SFvpd_Lthres, SFvpd_Uthres)
  dataWETF <- calculateGPP_VPDStress(dataWETF, RUE=RUE, Lvpd=Lvpd, Uvpd=Uvpd, 
                                     SFvpd_Lthres=SFvpd_Lthres, SFvpd_Uthres=SFvpd_Uthres)
  #yieldWETF <- getYield(dataWETF, TFname=TFname, phenoVars, is_VPDStress=T, shrtReport=T)
  yieldWETF <- getYield(dataWETF, phenoVars, is_VPDStress=T)
  
  #r_yieldWETF <- reportYield(yieldWETF, TFname=TFname, phenoVars, data_stats, is_VPDStress=T, shrtReport=T, saveReport=F, saveFig=F)
  r_yieldWETF <- reportYield(yieldWETF, RUE=RUE, Tmin=Tmin, Topt=Topt, Tmax=Tmax, TminFactor=TminFactor, phenoVars, data_stats, 
  ...
  # df_stats <- generateStats_v3(r_yieldWETF, obs_fld='ObsYield', TFname=TFname, 
  #                              RUE, TminFactor, Tmin, Toptmin, Topt, Toptmax, Tmax, 
  #                              Lvpd, Uvpd, SFvpd_Lthres, SFvpd_Uthres, is_VPDStress=T)
  
  df_stats <- createDF(r_yieldWETF, RUE=RUE, TminFactor=TminFactor, Tmin=Tmin, 
                       Topt=Topt, Tmax=Tmax, pred_fld=pred_fld, is_VPDStress=T, 
...
# --------------------------------
# Estimate GPP
# dataWETF <- calculateGPP(data, RUE, TminFactor, Tmin, Topt, Tmax)
# # Estimate Yield
# yieldWETF <- getYield(dataWETF, phenoVars)
# # Report & Chart
# r_yieldWETF <- reportYield(yieldWETF, RUE, Tmin, Topt, Tmax, TminFactor, phenoVars, data_stats, shrtReport=T, saveReport=T, mergeReport=T, saveFig=T)
# # Display Figure
# chartYieldxmodel(r_yieldWETF, TFname="WETF", grp='loc_code', subtitle="WETF", fname="WETF", saveFig=F)
# ------------- END TESTING -------------------
...
# WETF
# ---------------------------------------------
# 1) Combinations for RUE, TminFactor, Tmin, Topt, Tmax in WETF (no stress)
combination_args_WETF_noStress <- list(
  RUE = c(3.0), #c(seq(2.8, 3.2, 0.1)),
  Tmin = c(0:5),
  Topt = c(10:25),
  Tmax = c(30:50),
  TminFactor = c(0.1, 0.2, 0.25, 0.3, 0.4, 0.5)
)
cmb_list_WETF_noStress <- getAllCombinations_WETF_noStress(combination_args_WETF_noStress)

start <- proc.time()
cl <- parallel::makeCluster( parallel::detectCores() - 1,  type = "FORK" ) #"PSOCK" #FORK 
doParallel::registerDoParallel(cl = cl)
df_result_combinations_WETF_noStress <- foreach( i = cmb_list_WETF_noStress, .packages = c('dplyr'), 
                                                 .combine = 'rbind' ) %dopar% {
   tryCatch({
     metrics_combinations_WETF_noStress(i, data, phenoVars, data_stats, saveReport=F, mergeReport=T)
   }, error = function(e) return(paste0("Error occurred for '", i, "'", " The error is '", e, "'")))
 }
saveMetricsResults(df_result_combinations_WETF_noStress, "combinations_WETF_noStress.csv")
parallel::stopCluster(cl = cl)
print(proc.time() - start)
# user  system elapsed 
# 13.365   2.856 806.043 # ~30min
# ========================== END RUN IN PARALLEL ====================================

# -------------------------
# READ MERGED RESULTS
# -------------------------
combinations_Yield_WETF <- read.csv (paste(WD,OUTPUT_DIR,"/",Combinations_csv, sep="", collapse = NULL), check.names = FALSE, stringsAsFactors = FALSE)
View(combinations_Yield_WETF)



# ========================== VPD stress ====================================
# 2) Combinations for Tmin, Topt, Tmax in WETF (VPD stress)
combination_args_WETF_vpdStress <- list(
  RUE = c(3.0),
  Tmin = c(0:9),
  Topt = c(15:25),
  Tmax = c(30:50),
...
cmb_list_WETF_vpdStress <- getAllCombinations_WETF_vpdStress(combination_args_WETF_vpdStress, rtrnList=F)
...
doParallel::registerDoParallel(cl = cl)
# Using the dataframe
df_result_combinations_WETF_vpdStress <- foreach( i = 1:nrow(cmb_list_WETF_vpdStress), .packages = c('dplyr'), .combine = 'rbind' ) %dopar% {
  tryCatch({
    r <- cmb_list_WETF_vpdStress[i, ]
    metrics_combinations_WETF_vpdStress(r, data, phenoVars, data_stats, saveReport=F, mergeReport=T)
  }, error = function(e) return(paste0("Error occurred for '", i, "'",
                                       " The error is '", e, "'")))
}
# df_result_combinations_WETF_vpdStress
saveMetricsResults(df_result_combinations_WETF_vpdStress, "combinations_WETF_vpdStress.csv")
parallel::stopCluster(cl = cl)
print(proc.time() - start)
# user  system elapsed 
# 617.445   200.721 38854.599 
# For 258720 obs of 7 variables ~ 10 hrs

combinations_Yield_WETF_vpdStress <- read.csv (paste(WD,OUTPUT_DIR,"/",Combinations_csv, sep="", collapse = NULL), check.names = FALSE, stringsAsFactors = FALSE)
View(combinations_Yield_WETF_vpdStress)
